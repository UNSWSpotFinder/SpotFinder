...
export function UserRegistPage(){
    ...
    function verifyEmail(){
        if(code==='123456'){
            ...
            setOpenSnackbar({
                severity: 'warning',
                message: 'Incorrect verification code', // 在单词之间添加空格
            })
            console.log('warning');
            console.log(snackbarData.message);
        }
    }
    function Regist() {
    ...
    return(
        <div className='overall'>
            ...
            <motion.div className='contentoverall'
             ...
             transition={{ duration: 0.3 }}>
                <div className='back'>
                    <button onClick={backhome} type='button' className='backbtn'>×</button>
                </div>
                <div className='contentmain'>
                <img src='img/LOGO.svg' height={'100 px'} className='Logo'></img>
                <div className='LoginUser'>
                    <p className='welcomemain'>Welcome to Join SpotFinder</p>
                    <p className='welcomesub'>What is your reason for signing up today?</p>
                    <form className='w-100'>
                        <div className="form-check alcenter">
                            <input className="checker" type="radio"  checked={target === 'parking'} onChange={() => handleOptionChange('parking')}></input>
                            <label className="welcomesub zeropd" >
                                I am looking for parking space.
                            </label>
                        </div>
                        <div className="form-check alcenter mb-4">
                            <input className="checker" type="radio" checked={target === 'earning'} onChange={() => handleOptionChange('earning')}></input>
                            <label className="welcomesub zeropd">
                        ...
                        <div className='formpart-col'>
                            ...
                            <div className='emailverify'>
                                <input type="email" className="form-control rightcontrol-r setmargin" disabled={isverifyed} onChange={handleEmailChange} value={Email}></input>
                        ...
                        <div className='formpart-col'>
                            ...
                            <input className="form-control rightcontrol setmargin" value={phone} onChange={handlePhoneChange}></input>
                        </div>
                        <div className='formpart-col'>
                            <label className="form-label title">Preferred name</label>
                            <input className="form-control rightcontrol setmargin" value={name} onChange={handleNameChange} ></input>
                        </div>
                        <div className='formpart-col'>
                        <label className="form-label title">Date of Birth</label>
                            <LocalizationProvider dateAdapter={AdapterDayjs}>
                                <DatePicker className="setmargin daterightcontrol title" label="DateBirth" value={BirthDate} onChange={handleBirthdayChange}></DatePicker>
                        ...
                        <div className='formpart-row'>
                            <div className='seepart'>
                                <input type="checkbox" onChange={()=>setPasswordVisibility1(!passwordVisibility1)} className='checks'></input>
                                <label className="pwdsee" htmlFor="exampleCheck1">show password</label>
                            </div>
                        </div>
                        <div className='formpart-col'>
                            <label className="form-label title">Password Confirmation</label>
                            <input type={passwordVisibility2? "text": "password"} 
                            autoComplete="new-password" 
                            className="form-control rightcontrol setmargin" 
                            value={Password1} onChange={handlePassword1Change}></input>
                        </div>
                        <div className='formpart-row'>
                            <div className='seepart'>
                            <input type="checkbox" onChange={()=>setPasswordVisibility2(!passwordVisibility2)} className='checks'></input>
                            <label className="pwdsee" htmlFor="exampleCheck1">show password</label>
                            </div>
                        </div>
                        <div className='formpart-img'>
                            <img src={selectedFileName} className='profilephoto' width="100 px" height="100px"></img>
                            <input type="file" id="fileInput" style={{ display: 'none' }} onChange={handleFileChange} />
                            <button onClick={EditPhoto} type="button" className='changeprofile'>Select your profile photo</button>
                        </div>
                        <button type="button" className="btn btn-primary logbtn">SIGN UP</button>
                    </form>
                    <div className='logbottom'>
                        <p className='welcomesub'>Already have an account?</p>
                        <a className='forget' onClick={goeslogin}>Click here to Sign in</a>
                    </div>
